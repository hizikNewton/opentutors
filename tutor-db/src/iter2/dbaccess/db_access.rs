...
pub async fn get_courses_for_tutor_db(
    ...
    let course_rows = sqlx::query_as!(Course,"SELECT * FROM ezy_course_c4 where tutor_id = $1",tutor_id)
    .fetch_all(pool)
    .await?;
    Ok(course_rows)
}
...
pub async fn get_course_details_db(
    ...
    let course_row = sqlx::query_as!(Course, 
        "SELECT * FROM ezy_course_c4 where tutor_id = $1 and course_id = $2",tutor_id,course_id).fetch_optional(pool).await?;
...
pub async fn post_new_course_db(
    ...
    let course_row= sqlx::query_as!(Course,"insert into ezy_course_c4 (
...
pub async fn delete_course_db(
    ...
    course_id: i32,
        ...
        let course_row = sqlx::query!(
            "DELETE FROM ezy_course_c4 where tutor_id = $1 and course_id = $2",
            tutor_id,course_id).execute(pool)
            .await?;
...
pub async fn update_course_details_db(
    ...
    let current_course_row = sqlx::query_as!(
        ...
        tutor_id,
        course_id).fetch_one(pool).await.map_err(|_err| EzyTutorError::NotFound("Course id not found".into()))?;
